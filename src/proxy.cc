...
static bool NeedProxy(int type, int pattern, int root, struct ncclRing *ring,
                      int nranks) {
...
static inline void addUniqueBinePeer(int peer, int rank, int channelId,
...
static ncclResult_t expectedProxyResponseEnqueue(struct ncclProxyState *state,
                                                 void *opId, int respSize) {
...
static ncclResult_t expectedProxyResponseRemove(struct ncclProxyState *state,
                                                void *opId) {
  struct ncclExpectedProxyResponse *elem = state->expectedResponses;
  struct ncclExpectedProxyResponse *prev = NULL;
  while (elem) {
...
static ncclResult_t asyncProxyOpEnqueue(struct ncclProxyLocalPeer *peer,
                                        ncclProxyAsyncOp *op) {
...
static ncclResult_t asyncProxyOpDequeue(struct ncclProxyLocalPeer *peer,
                                        ncclProxyAsyncOp *op) {
...
static ncclResult_t allocateArgs(struct ncclProxyProgressState *state,
...
ncclResult_t printProxyOp(struct ncclProxyArgs *op, int poolIndex,
                          int opIndex) {
...
static ncclResult_t ncclProxyOpToArgs(struct ncclProxyOp *op,
  ...
  if (subIndex) {
    ...
    if ((args->sliceSteps != op->sliceSteps) ||
        ...
        (args->protocol != op->protocol) || (args->dtype != op->dtype) ||
        (args->redOp != op->redOp) || (args->coll != op->coll)) {
  ...
  if (args) {
    if (shared && args->opCount == op->opCount) {
      ...
      DEBUG_PROXY_PRINT("Insert (%d/%5ld/%5ld) as group with %5ld\n", shared,
                        args->opCount, op->opCount, OP_INDEX(args));
    } else {
...
ncclResult_t ncclProxyPost(struct ncclProxyOpsPool *pool, int nextOps,
                           int nextOpsEnd) {
...
static ncclResult_t ncclLocalOpAppend(struct ncclComm *comm,
  ...
  if (opIndex != -1) {
    ...
    // Read the freeOps value and wait for a value different than -1. Once not
    // -1, read the value with acquire and reset -1
    int freeOp = -1;
    while (freeOp == -1) {
  ...
  if (op->next != -1)
    COMPILER_PREFETCH(pool->ops + op->next); // Prefetch next free op
  ...
  if (++proxyOps->count == MAX_OPS_PER_PEER) {
    ...
    // Do not post last operations as we could have more coming with the same
    // opCount, and posting them in different batches would break proxyArgs
    // aggregation with subs.
    uint64_t lastOpCount = pool->ops[proxyOps->nextOpsEnd].opCount;
    int lastOp = -1;
    int toSend = 0;
    int ops = 0;
    for (int op = proxyOps->nextOps; op != proxyOps->nextOpsEnd;
         op = pool->ops[op].next) {
      ops++;
...
static void incWorkCounter(struct ncclComm *comm, struct ncclProxyOp *op) {
  op->workCounter = (op->incWorkCounter)
                        ? ++comm->profiler.workCounter[op->channelId]
                        : comm->profiler.workCounter[op->channelId];
}

static ncclResult_t SaveProxyProfiler(struct ncclComm *comm,
                                      struct ncclProxyOp *op,
                                      bool *justInquire) {
  struct ncclProxyConnector *proxyConn =
      (op->coll == ncclFuncRecv) ? &comm->profiler.recvProxyConn[op->channelId]
  ...
  if (justInquire) {
    *justInquire = true;
    ...
    // Ensure that in graph capturing the proxy workCounter is incremented to
    // keep up with kernel workCounter
    if (comm->planner.persistent)
      incWorkCounter(comm, op);
...
static ncclResult_t SaveProxy(struct ncclComm *comm,
                              struct ncclChannel *channel, int type, int peer,
                              struct ncclProxyOp *op, int connIndex,
                              bool *justInquire) {
  if (peer < 0)
    return ncclSuccess;

  struct ncclChannelPeer *peerComm = channel->peers[peer];
  struct ncclConnector *connector = type == proxyRecv
                                        ? peerComm->recv + connIndex
                                        : peerComm->send + connIndex;
...
// justInquire != nullptr means don't actually do anything, just assertain need
// of ncclProxySaveOp for this op.
ncclResult_t ncclProxySaveOp(struct ncclComm *comm, struct ncclProxyOp *op,
  ...
  case ncclPatternPipelineTo: {
    ...
    if (needProxy)
      NCCLCHECK(
          SaveProxy(comm, channel, proxyRecv, ring->prev, op, 0, justInquire));
    ...
    if (needProxy)
      NCCLCHECK(
          SaveProxy(comm, channel, proxySend, ring->next, op, 0, justInquire));
  } break;
  ...
  case ncclPatternTreeUpDown: {
    if (op->pattern != ncclPatternTreeDown) { // Tree up
      ...
      for (int i = 0; i < NCCL_MAX_TREE_ARITY; i++) {
        NCCLCHECK(SaveProxy(comm, channel, proxyRecv, tree->down[i], op, 0,
                            justInquire));
      }
      NCCLCHECK(
          SaveProxy(comm, channel, proxySend, tree->up, op, 0, justInquire));
    }
    if (op->pattern != ncclPatternTreeUp) { // Tree down
      struct ncclTree *tree = &channel->tree;
      for (int i = 0; i < NCCL_MAX_TREE_ARITY; i++) {
        NCCLCHECK(SaveProxy(comm, channel, proxySend, tree->down[i], op, 0,
                            justInquire));
      }
      NCCLCHECK(
          SaveProxy(comm, channel, proxyRecv, tree->up, op, 0, justInquire));
    }
  } break;
  ...
  case ncclPatternCollnetChain: {
    NCCLCHECK(SaveProxy(comm, channel, proxySend, channel->collnetChain.up, op,
                        1, justInquire));
    NCCLCHECK(SaveProxy(comm, channel, proxyRecv, channel->collnetChain.up, op,
                        0, justInquire));
  } break;
  case ncclPatternCollnetDirect: {
    NCCLCHECK(SaveProxy(comm, channel, proxySend, channel->collnetDirect.out,
                        op, 1, justInquire));
    NCCLCHECK(SaveProxy(comm, channel, proxyRecv, channel->collnetDirect.out,
                        op, 0, justInquire));
  } break;
  case ncclPatternNvls: {
    NCCLCHECK(SaveProxy(comm, channel, proxySend, channel->nvls.out, op, 1,
                        justInquire));
    NCCLCHECK(SaveProxy(comm, channel, proxyRecv, channel->nvls.out, op, 0,
                        justInquire));
  } break;
  case ncclPatternNvlsTree: {
    NCCLCHECK(SaveProxy(comm, channel, proxyRecv, channel->nvls.treeDown[1], op,
                        0, justInquire));
    NCCLCHECK(SaveProxy(comm, channel, proxyRecv, channel->nvls.treeDown[2], op,
                        0, justInquire));
    NCCLCHECK(SaveProxy(comm, channel, proxySend, channel->nvls.treeUp, op, 0,
                        justInquire));
    NCCLCHECK(SaveProxy(comm, channel, proxySend, channel->nvls.treeDown[1], op,
                        0, justInquire));
    NCCLCHECK(SaveProxy(comm, channel, proxySend, channel->nvls.treeDown[2], op,
                        0, justInquire));
    NCCLCHECK(SaveProxy(comm, channel, proxyRecv, channel->nvls.treeUp, op, 0,
                        justInquire));
  } break;
  case ncclPatternPatUp: {
    ...
    PatRSAlgorithm<char> algo(op->chunkSize, NCCL_STEPS, 16, 0, size, size,
    ...
    for (int i = 0; i < log2Up(nranks); i++) {
      if (nstepsSend[i]) {
        ...
        NCCLCHECKGOTO(
            SaveProxy(comm, channel, proxySend, sendPeer, op, 0, justInquire),
            result, exit_pat_up);
      }
      if (nstepsRecv[i]) {
        ...
        NCCLCHECKGOTO(
            SaveProxy(comm, channel, proxyRecv, recvPeer, op, 0, justInquire),
            result, exit_pat_up);
      }
    }
  exit_pat_up:
  ...
  case ncclPatternPatDown: {
    ...
    const int rank = comm->rank, nranks = comm->nRanks;
    int *nstepsSend = NULL, *nstepsRecv = NULL;
    PatAGAlgorithm<char> algo(op->chunkSize, NCCL_STEPS, 16, 0, size, size,
                              op->chunkSize, rank, nranks);
    struct ncclPatStep ps = {0};
    NCCLCHECKGOTO(ncclCalloc(&nstepsSend, log2Up(nranks)), result,
                  exit_pat_down);
    NCCLCHECKGOTO(ncclCalloc(&nstepsRecv, log2Up(nranks)), result,
                  exit_pat_down);
  ...
  case ncclPatternRecv: {
    if (op->root == comm->rank)
      return ncclSuccess;
    NCCLCHECK(SaveProxy(comm, channel,
                        op->pattern == ncclPatternSend ? proxySend : proxyRecv,
                        op->root, op, 1, justInquire));
  } break;
...
static ncclResult_t removeOp(struct ncclProxyProgressState *state,
  ...
  *opPtr = next;
  ...
  state->pool = freeOp;
  DEBUG_PROXY_PRINT("Removed %5ld (%5ld) : ", OP_INDEX(freeOp),
                    OP_INDEX(*freeOp->proxyAppendPtr));
#ifdef DEBUG_PROXY
...
static ncclResult_t ncclProxyGetPostedOps(struct ncclProxyState *proxyState,
  ...
  // arrive or even wait for the lock to be available. Exit, continue progress,
  // and come back later.
  {
    ...
    if (state->active == NULL) {
      ...
      if (pool->nextOps == -1 && !state->stop) {
        ...
        ncclProfilerRecordProxyCtrlEventState(eHandle, 0,
...
process_nextops:
  ...
  for (int opIndex = state->nextOps; opIndex != -1;) {
    ...
    if (peerOp->connection == NULL)
      return ncclInternalError;
    if (peerOp->next != -1)
      COMPILER_PREFETCH(pool->ops + peerOp->next);
  ...
  for (int i = 0; i < proxyState->tpLocalnRanks; i++) {
    ...
    // prepend the ops freeOp[i]-freeOpEnd[i] in front of the pool->freeOps[i]
    // op
    oldFree =
        COMPILER_ATOMIC_LOAD(&pool->freeOps[i], std::memory_order_acquire);
    do {
      // Coverity gets confused by the complex code structure here.  The
      // previous "for" loop ensures that freeOpEnd[i] is initialized so long as
      // freeOp[i] is initialized (is not -1).  In the current loop we filter
      // out uninitialized freeOp[i], hence ensuring that freeOpEnd[i] is also
      // initialized. coverity[uninit_use:FALSE]
      pool->ops[freeOpEnd[i]].next = oldFree;
    } while (!COMPILER_ATOMIC_COMPARE_EXCHANGE(
        &pool->freeOps[i], &oldFree, newFree,
        /*success=*/std::memory_order_release,
        /*failure=*/std::memory_order_acquire));
  }
  ncclProfilerRecordProxyCtrlEventState(eHandle, *added,
...
void ncclDumpProxyState(int signal) { dumpProxyState(ncclLastProxyState); }
...
static ncclResult_t setProxyThreadContext(struct ncclProxyState *proxyState,
  ...
  // Driver checks and cuCtxCreate inside the callable to ensure visibility
  // across threads.
  std::call_once(proxyState->cudaCtxOnceFlag, [&]() {
    if (CUPFN(cuCtxCreate) == nullptr || CUPFN(cuCtxDestroy) == nullptr ||
        CUPFN(cuCtxSetCurrent) == nullptr) {
      INFO(NCCL_INIT,
           "[%s] Unable to create thread context due to old driver, disabling "
           "cuda context.",
           prefix);
      proxyState->cudaCtx = NULL;
      return;
    }
    int cudaError = CUPFN(cuCtxCreate(&proxyState->cudaCtx, NULL, 0,
                                      CU_CTX_SCHED_SPIN | CU_CTX_MAP_HOST,
                                      proxyState->cudaDev));
    if (cudaError != CUDA_SUCCESS) {
      INFO(NCCL_INIT,
           "[%s] Failed to create CUDA context on device %d with error %d, "
           "disabling cuda context.",
           prefix, proxyState->cudaDev, cudaError);
      proxyState->cudaCtx = NULL;
    }
  });
  ...
  // Make the context current in this thread. cuCtxCreate already did this for
  // the creating thread, but all three proxy threads call here so keep them
  // symmetric.
  int cudaError = CUPFN(cuCtxSetCurrent(proxyState->cudaCtx));
  if (cudaError != CUDA_SUCCESS) {
    INFO(NCCL_INIT,
         "[%s] Failed to set CUDA context on device %d with error %d.", prefix,
...
void proxyCpusetOnceFunc() {
  ...
  if (setEnv) {
    ...
    (void)ncclCpusetToStrList(&proxyCpuset, msg + strlen(msg),
...
void *ncclProxyProgress(void *proxyState_) {
  ...
  // This thread is created by proxyService, therefore setting the affinity is
  // not needed.
  INFO(NCCL_INIT, "[Proxy Progress] Device %d CPU core %d", proxyState->cudaDev,
       ncclOsGetCpu());
  ...
  do {
    ...
    if (ret != ncclSuccess) {
      COMPILER_ATOMIC_STORE(&proxyState->asyncResult, ret,
    ...
    if (idle || !state->active ||
      ...
      if (ret != ncclSuccess) {
        COMPILER_ATOMIC_STORE(&proxyState->asyncResult, ret,
                              std::memory_order_release);
        INFO(NCCL_ALL, "%s:%d -> %d [Progress Thread]", __FILE__, __LINE__,
             ret);
      }
      if (added == 0) {
    ...
    lastIdle = idle;
           ...
           COMPILER_ATOMIC_LOAD(proxyState->abortFlag,
...
ncclResult_t ncclProxyStart(struct ncclComm *comm) {
  ...
  if (proxyOps == NULL)
    return ncclSuccess;
  TIME_START(1);
  // Use peerArraySize which tracks actual allocated size (may be nRanks for
  // cross-clique)
...
static ncclResult_t ncclProxyNewConnection(struct ncclProxyConnectionPool *pool,
                                           int *id) {
...
static ncclResult_t ncclProxyGetConnection(struct ncclProxyConnectionPool *pool,
                                           int id,
...
static ncclResult_t proxyFree(struct ncclProxyConnection *connection,
  ...
  if (connection->send) {
    ...
    if (ncclTransports[connection->transport]->recv.proxyFree) {
      NCCLCHECK(ncclTransports[connection->transport]->recv.proxyFree(
          connection, proxyState));
    }
  }
  return ncclSuccess;
...
static ncclResult_t
ncclProxyFreeConnections(struct ncclProxyConnectionPool *pool,
                         struct ncclProxyState *proxyState) {
...
ncclResult_t ncclProxyConnect(struct ncclComm *comm, int transport, int send,
  ...
  if (sharedProxyState->peerSocks == NULL) {
    int peerArraySize =
        comm->p2pCrossClique ? comm->nRanks : comm->sharedRes->tpNLocalRanks;
  ...
  int peerIndex = comm->p2pCrossClique
                      ? proxyConn->tpRank
                      : comm->sharedRes->tpRankToLocalRank[proxyConn->tpRank];
  ...
  if (!ready) {
    NCCLCHECK(ncclSocketInit(
        sock, sharedProxyState->peerAddresses + proxyConn->tpRank,
  ...
  // This usually sends proxyConn->connection to identify which connection this
  // is. However, this is part of the response and therefore is ignored
  NCCLCHECK(ncclProxyCallBlocking(comm, proxyConn, ncclProxyMsgInit, &req,
                                  sizeof(req), &resp, sizeof(resp)));
  proxyConn->connection = resp.connection;

  // If we need proxy progress, map progress ops
  struct ncclTransportComm *tcomm = send ? &ncclTransports[transport]->send
                                         : &ncclTransports[transport]->recv;
  if (tcomm->proxyProgress) {
    ...
    struct ncclProxyOps *proxyOps =
        sharedProxyState->proxyOps + proxyConn->tpLocalRank;
    if (proxyOps->pool == NULL) {
      NCCLCHECK(ncclShmOpen(
          poolPath, sizeof(poolPath), sizeof(struct ncclProxyOpsPool),
  ...
  INFO(NCCL_PROXY, "Connected to proxy localRank %d -> connection %p",
       proxyConn->tpLocalRank, proxyConn->connection);
  return ncclSuccess;
}
...
ncclResult_t ncclProxyCallBlockingUDS(struct ncclComm *comm,
  ...
  INFO(NCCL_PROXY,
       "ProxyCall UDS comm %p rank %d tpRank %d(%lx) reqSize %d respSize %d "
       "respFd %p opId %p",
  ...
  NCCLCHECKGOTO(ncclIpcSocketSendMsg(&ipcSock, &hdr, sizeof(hdr), reqFdtmp,
                ...
                res, error);
  NCCLCHECKGOTO(ncclIpcSocketRecvMsg(&ipcSock, respBuff, respSize, respFd), res,
                error);
  NCCLCHECKGOTO(ncclIpcSocketClose(&ipcSock), res, error);

  INFO(NCCL_PROXY,
       "ProxyCall UDS comm %p rank %d tpRank %d(%lx) reqSize %d respSize %d "
       "respFd %d opId %p - DONE",
       comm, rank, proxyConn->tpRank, pidHash, reqSize, respSize,
       (respFd ? *respFd : -1), opId);

  return res;

error:
  NCCLCHECK(ncclIpcSocketClose(&ipcSock));
  WARN("ncclProxyCallBlockingUDS call to tpRank %d(%lx) failed : %d",
       proxyConn->tpRank, pidHash, res);
  return res;
}

// cuMem API support
// The request/response is sent out-of-band using ncclIpcSocket for this
// specific command
ncclResult_t ncclProxyClientGetFdBlocking(struct ncclComm *comm, int proxyRank,
  ...
  if (comm->gproxyConn[proxyRank].initialized == false) {
    NCCLCHECKGOTO(ncclProxyConnect(comm, TRANSPORT_P2P, 1, proxyRank,
                                   &comm->gproxyConn[proxyRank]),
                  ret, error);
  }
  NCCLCHECKGOTO(ncclProxyCallBlockingUDS(comm, &comm->gproxyConn[proxyRank],
                                         ncclProxyMsgGetFd, handle,
                                         sizeof(CUmemGenericAllocationHandle),
                                         NULL, 0, NULL, convertedFd),
                ret, error);

  // We have now received the converted fd over UDS
  INFO(NCCL_PROXY,
       "UDS: ClientGetFd handle 0x%lx tpRank %d returned fd %d sameProcess %d",
       *(uint64_t *)handle, comm->topParentRanks[proxyRank], *convertedFd,
       comm->gproxyConn[proxyRank].sameProcess);

  return ret;

error:
  WARN("ncclProxyClientGetFd call to tpRank %d handle 0x%lx failed : %d",
       comm->topParentRanks[proxyRank], *(uint64_t *)handle, ret);
  return ret;
}

ncclResult_t ncclProxyClientBatchQueryFdBlocking(
    struct ncclComm *comm, struct ncclProxyConnector *proxyConn, int *localFds,
    int *rmtFds, int numSegments) {
  ncclResult_t ret = ncclSuccess;
  for (int segment = 0; segment < numSegments; segment++) {
    NCCLCHECKGOTO(ncclProxyCallBlockingUDS(comm, proxyConn, ncclProxyMsgQueryFd,
                                           NULL, 0, (void *)&rmtFds[segment],
                                           sizeof(int), &localFds[segment],
                                           NULL),
                  ret, fail);
    // We have now received the converted fd for a segment over UDS
    INFO(NCCL_PROXY,
         "UDS: ClientQueryFdBatch localFd %d tpRank %d remote fd %d "
         "sameProcess %d segment %d",
         localFds[segment], proxyConn->tpRank, rmtFds[segment],
         proxyConn->sameProcess, segment);
  }
exit:
  return ret;
fail:
...
ncclProxyClientQueryFdBlocking(struct ncclComm *comm,
  ...
  ncclResult_t ret = ncclSuccess;
  NCCLCHECKGOTO(ncclProxyCallBlockingUDS(comm, proxyConn, ncclProxyMsgQueryFd,
                                         NULL, 0, (void *)rmtFd, sizeof(int),
                                         &localFd, NULL),
                ret, fail);
exit:
  // We have now received the converted fd over UDS
  INFO(NCCL_PROXY,
       "UDS: ClientQueryFd localFd %d tpRank %d remote fd %d sameProcess %d",
       localFd, proxyConn->tpRank, *rmtFd, proxyConn->sameProcess);
  return ret;
fail:
  WARN(
      "ncclProxyClientQueryFdBlocking call to tpRank %d localFd %d failed : %d",
      proxyConn->tpRank, localFd, ret);
  goto exit;
}

const char *ncclProxyMsgTypeStr[] = {
    "Unknown", "Init",     "SharedInit", "Setup", "Connect",
    "Start",   "Close",    "Abort",      "Stop",  "GetFd",
    "QueryFd", "Register", "Deregister"};
ncclResult_t ncclProxyCallAsync(struct ncclComm *comm,
  ...
  NCCLCHECKGOTO(ncclSocketSend(sock, &proxyConn->connection, sizeof(void *)),
                ret, error);
...
ncclResult_t ncclPollProxyResponse(struct ncclComm *comm,
  ...
  if (found == 0) {
    ...
    struct ncclSocket *sock =
        sharedProxyState->peerSocks + proxyConn->tpLocalRank;
    ncclProxyRpcResponseHeader resp = {0};
    int offset = 0;
    if (ncclSuccess != ncclSocketProgress(NCCL_SOCKET_RECV, sock, &resp,
                                          sizeof(resp), &offset)) {
    ...
    if (offset == 0) {
      ...
      while (offset < sizeof(resp))
        NCCLCHECK(ncclSocketProgress(NCCL_SOCKET_RECV, sock, &resp,
                                     sizeof(resp), &offset));
    }
    ...
    if (resp.opId == opId) {
      INFO(NCCL_PROXY, "resp.opId=%p matches expected opId=%p", resp.opId,
           opId);
      NCCLCHECK(expectedProxyResponseRemove(sharedProxyState, resp.opId));
      return resp.res;
    } else {
      INFO(NCCL_PROXY, "Queuing opId=%p respBuff=%p respSize=%d", resp.opId,
           respBuff, resp.respSize);
      // Store the result and mark response as completed
      NCCLCHECK(expectedProxyResponseStore(sharedProxyState, resp.opId,
                                           respBuff, resp.respSize, resp.res));
      return ncclInProgress;
    }
  } else {
...
ncclResult_t ncclProxyCallBlocking(struct ncclComm *comm,
  ...
  NCCLCHECKGOTO(ncclProxyCallAsync(comm, proxyConn, type, reqBuff, reqSize,
                                   respSize, opId),
                res, fail);
...
static ncclResult_t proxyProgressInit(struct ncclProxyState *proxyState) {
  ...
  if (state->opsPool == NULL) {
    ...
    snprintf(shmPath, sizeof(shmPath), "Local\\nccl-shm-%d",
             GetCurrentProcessId());
#endif
    NCCLCHECK(ncclShmOpen(shmPath, sizeof(shmPath), size, (void **)&pool, NULL,
                          proxyState->tpLocalnRanks, &state->handle));
    // Init pool
    pool->nextOps = -1;
...
#if defined(NCCL_OS_LINUX)
    memcpy(state->opsPoolShmSuffix, shmPath + sizeof("/dev/shm/nccl-") - 1,
           sizeof("XXXXXX") - 1);
#elif defined(NCCL_OS_WINDOWS)
...
static ncclResult_t
proxyConnInit(struct ncclProxyLocalPeer *peer,
              struct ncclProxyConnectionPool *connectionPool,
              struct ncclProxyState *proxyState, ncclProxyInitReq *req,
  ...
  COMPILER_ATOMIC_STORE(&(*connection)->state, connInitialized,
                        std::memory_order_release);
  return ncclSuccess;
}
...
static ncclResult_t proxyQueryFd(struct ncclProxyState *proxyState, int rank,
  ...
  NCCLCHECKGOTO(ncclIpcSocketInit(&ipcSock, proxyState->tpRank, hash ^ 1,
                                  proxyState->abortFlag),
                ret, exit);
  NCCLCHECKGOTO(
      ncclIpcSocketSendMsg(&ipcSock, &rmtFd, sizeof(int), -1, rank, hash), ret,
      exit);
exit:
  NCCLCHECK(ncclIpcSocketClose(&ipcSock));
  return ncclSuccess;
...
static ncclResult_t
proxyProgressAsync(struct ncclProxyAsyncOp *op,
  ...
  if (op->type == ncclProxyMsgSetup) {
    ...
    TRACE(NCCL_PROXY,
          "proxyProgressAsync::proxyConnect() opId=%p op.reqBuff=%p", op->opId,
          op->reqBuff);
    res = op->connection->tcomm->proxyConnect(
        op->connection, proxyState, op->reqBuff, op->reqSize, op->respBuff,
        op->respSize, &done);
  } else if (op->type == ncclProxyMsgSharedInit) {
    int nChannels = (int)*op->reqBuff;
    TRACE(NCCL_PROXY,
          "proxyProgressAsync::ncclProxyMsgSharedInit opId=%p op.reqBuff=%p "
          "nChannels=%d",
          op->opId, op->reqBuff, nChannels);
    if (op->connection->tcomm->proxySharedInit)
      res = op->connection->tcomm->proxySharedInit(op->connection, proxyState,
                                                   nChannels);
    COMPILER_ATOMIC_STORE(&op->connection->state,
                          static_cast<proxyConnectState>(connSharedInitialized),
                          std::memory_order_release);
  } else if (op->type == ncclProxyMsgInit) {
    TRACE(NCCL_PROXY,
          "proxyProgressAsync::ncclProxyMsgInit opId=%p op.reqBuff=%p",
          op->opId, op->reqBuff);
    ...
    TRACE(NCCL_PROXY,
          "proxyProgressAsync::ncclProxyMsgRegister opId=%p op.reqBuff=%p, "
          "op->reqSize=%d, op->respSize=%d",
          op->opId, op->reqBuff, op->reqSize, op->respSize);
    res = op->connection->tcomm->proxyRegister(
        op->connection, proxyState, op->reqBuff, op->reqSize, op->respBuff,
        op->respSize, &done);
  } else if (op->type == ncclProxyMsgDeregister) {
    TRACE(NCCL_PROXY,
          "proxyProgressAsync::ncclProxyMsgDeregister opId=%p op.reqBuff=%p, "
          "op->reqSize=%d, op->respSize=%d",
          op->opId, op->reqBuff, op->reqSize, op->respSize);
    res = op->connection->tcomm->proxyDeregister(
        op->connection, proxyState, op->reqBuff, op->reqSize, &done);
  } else
    return ncclInternalError;
  ...
  if (done) {
    ...
    if (op->type == ncclProxyMsgSetup)
      COMPILER_ATOMIC_STORE(&op->connection->state,
                            static_cast<proxyConnectState>(connSetupDone),
                            std::memory_order_release);
    else if (op->type == ncclProxyMsgConnect)
      COMPILER_ATOMIC_STORE(&op->connection->state,
                            static_cast<proxyConnectState>(connConnected),
                            std::memory_order_release);
    /* if setup or connect is done, we should not return any error at this point
     * since ncclSocketSend might already send the respBuff to the requester. If
     * we still choose to abort and close the connection, it can cause segfault
     * if the requester is using the respBuff. */
  ...
  } else if (COMPILER_ATOMIC_LOAD(proxyState->abortFlag,
                                  std::memory_order_acquire) != 0) {
    return ncclInternalError;
  }
...
static ncclResult_t
proxyServiceInitOp(int type, struct ncclProxyLocalPeer *peer,
                   struct ncclProxyConnectionPool *connectionPool,
                   struct ncclProxyState *proxyState, int *asyncOpCount) {
  ncclResult_t ret = ncclSuccess;
  ...
  asyncOp->type = type;
  NCCLCHECKGOTO(ncclSocketRecv(sock, &asyncOp->connection, sizeof(void *)), ret,
                fail);

  NCCLCHECKGOTO(ncclSocketRecv(sock, &asyncOp->reqSize, sizeof(int)), ret,
                fail);
  NCCLCHECKGOTO(ncclSocketRecv(sock, &asyncOp->respSize, sizeof(int)), ret,
                fail);
  if (asyncOp->reqSize) {
    NCCLCHECKGOTO(ncclCalloc(&asyncOp->reqBuff, asyncOp->reqSize), ret, fail);
    NCCLCHECKGOTO(ncclSocketRecv(sock, asyncOp->reqBuff, asyncOp->reqSize), ret,
                  fail);
  }

  // Store opId for completion response
  NCCLCHECKGOTO(ncclSocketRecv(sock, &asyncOp->opId, sizeof(asyncOp->opId)),
                ret, fail);
  ...
  (*asyncOpCount)++;
  NCCLCHECK(proxyProgressAsync(asyncOp, proxyState, asyncOpCount, peer,
                               connectionPool));
exit:
  return ret;
fail:
...
enum { PROXY_RUNNING = 0, PROXY_STOP = 1, PROXY_ABORT = 2 };
...
void *ncclProxyService(void *_args) {
  ...
  INFO(NCCL_INIT, "[Proxy Service] Device %d CPU core %d", proxyState->cudaDev,
       ncclOsGetCpu());
  ...
  int maxProxyConnections =
      std::max(proxyState->tpnRanks + 1, NCCL_MAX_PROXY_CONNECTIONS);
  int maxnpeers = 0;
  int npeers = 0;
  ...
  NCCLCHECKGOTO(
      ncclSocketGetFd(proxyState->listenSock, &pollfds[maxProxyConnections].fd),
      ret, fail);
  ...
  while (stop == PROXY_RUNNING || npeers > 0) {
    /* Even if local comm aborts, we cannot let proxy thread exit if we still
     * have peer connections. Need to wait until all other related comms call
     * abort and safely exit together, or we could face segmentation fault. */
    if (COMPILER_ATOMIC_LOAD(proxyState->abortFlag,
                             std::memory_order_acquire) != 0)
      stop = PROXY_ABORT;
    /* never let proxy service thread blocks in poll, or it cannot receive
     * abortFlag. */
    int ret = 0;
    ...
    if (pollfds[maxProxyConnections].revents) {
      ...
      int s = 0;
      while (s < maxProxyConnections && pollfds[s].fd != NCCL_INVALID_SOCKET)
        s++;
      if (s == maxProxyConnections) {
        WARN("[Proxy service] Too many connections (%d max)",
             maxProxyConnections);
        goto fail;
      }
      if (maxnpeers < s + 1)
      ...
      if (ncclSocketAccept(&peers[s].sock, proxyState->listenSock) !=
          ncclSuccess) {
        INFO(NCCL_PROXY, "[Service thread] Accept failed %s", strerror(errno));
      } else {
        NCCLCHECKGOTO(ncclSocketGetFd(&peers[s].sock, &pollfds[s].fd), ret,
                      fail);
        pollfds[s].events = NCCL_POLLIN;
        pollfds[s].revents = 0;
        npeers++;
    ...
    for (int s = 0; s < maxnpeers; s++) {
      ...
      if (stop == PROXY_ABORT && ncclCuMemEnable() && ncclCuMemHostEnable() &&
          !proxyState->directMode &&
          COMPILER_ATOMIC_LOAD(&proxyState->stop, std::memory_order_acquire))
        closeConn = 1;
      ncclProxyAsyncOp *op = peer->asyncOps;
      while (op != nullptr) {
        ncclProxyAsyncOp *opnext =
            op->next; /* in case op is freed in proxyProgressAsync */
        type = op->type;
        // Coverity gets confused here by complex code structure.  Yes,
        // connectionPool.pools gets dereferenced, and while calling
        // proxyProgressAsync() connectionPool.pools is NULL, but that changes
        // before it's dereferenced. coverity[var_deref_model:FALSE]
        res = proxyProgressAsync(op, proxyState, &asyncOpCount, peer,
                                 &connectionPool);
        if (res == ncclSuccess || res == ncclInProgress) {
          ...
          // Res is a bad result
          closeConn = 1;
          WARN("[Service thread] Error encountered progressing operation=%s, "
      ...
      if (pollfds[s].revents & readableFlag) {
        int closed;
        res = ncclSocketTryRecv(sock, &type, sizeof(int), &closed,
                                false /*blocking*/);
        if (res != ncclSuccess && res != ncclInProgress) {
          if (!COMPILER_ATOMIC_LOAD(proxyState->abortFlag,
            ...
            WARN("[Service thread] Could not receive type from localRank %d, "
                 "res=%u, closed=%d",
                 peer->tpLocalRank, res, closed);
          closeConn = 1;
        } else if (closed) {
          INFO(NCCL_DESTROY | NCCL_NET | NCCL_PROXY,
               "[Service thread] Connection closed by localRank %d",
               peer->tpLocalRank);
          closeConn = 1;
          ...
          if (type == ncclProxyMsgStop) {
            ...
            res = proxyServiceInitOp(type, peers + s, &connectionPool,
                                     proxyState, &asyncOpCount);
          } else {
            WARN("[Service thread] Unknown command %d from localRank %d", type,
                 peer->tpLocalRank);
            closeConn = 1;
          }

          INFO(NCCL_PROXY, "Received and initiated operation=%s res=%d",
               ncclProxyMsgTypeStr[type], res);
        }
      ...
      if (res != ncclSuccess && res != ncclInProgress) {
        if (!COMPILER_ATOMIC_LOAD(proxyState->abortFlag,
                                  std::memory_order_relaxed))
          WARN("[Proxy Service %d] Failed to execute operation %s from rank "
               "%d, retcode %d",
               proxyState->tpRank, ncclProxyMsgTypeStr[type], peer->tpRank,
               res);
        closeConn = 1;
      }
      ...
      if (closeConn) {
        ...
        pollfds[s].fd = NCCL_INVALID_SOCKET;
        pollfds[s].events =
            0; // Clear events for invalid socket descriptors on Windows
        npeers--;
      }
    }
  }

  // Wait for all operations to complete and stop progress thread before freeing
  // any resource
...
static ncclResult_t proxyUDSRecvReq(struct ncclProxyState *proxyState,
                                    int reqFd) {
  ncclIpcHdr hdr;
  int rmtFd = -1;

  NCCLCHECK(
      ncclIpcSocketRecvMsg(&proxyState->ipcSock, &hdr, sizeof(hdr), &rmtFd));
  if (hdr.type == ncclProxyMsgGetFd) {
    // cuMem API support for non-UB case, and rmtFd is not used since UDS proxy
    // thread need to export fd from handle and send it back to the main thread
    // to import the buffer. We just need to close this dummy rmtFd.
    uint64_t handle = *(uint64_t *)hdr.data;
    INFO(NCCL_PROXY,
         "proxyUDSRecvReq::ncclProxyMsgGetFd rank %d opId %p handle=0x%lx",
         hdr.rank, hdr.opId, handle);
    close(rmtFd);
    return proxyGetFd(proxyState, hdr.rank, hdr.opId, handle);
  } else if (hdr.type == ncclProxyMsgQueryFd) {
    // remote main thread registers buffer into this rank, it querys rmtFd of
    // this rank through UDS and the rmtFd is returned unchanged back to remote
    // main thread which will use rmtFd to call into proxy service thread for
    // buffer registration.
...
void *ncclProxyServiceUDS(void *_args) {
  ...
  if (setProxyThreadContext(proxyState, "Proxy Service UDS") == ncclSuccess) {
    ...
    WARN("[Proxy Service UDS] Failed to set CUDA device %d",
         proxyState->cudaDev);
  }
...
ncclResult_t ncclProxyCreate(struct ncclComm *comm) {
   ...
   * comm->proxyState->thread is join()'d by commFree() in init.cc when the
   * refCount reduces down to 0. */
  ...
  if (proxyState->refCount == 1) {
    ...
    proxyState->memManager =
        comm->memManager; // Set shared memory manager for proxy allocations
    ...
    ncclSetThreadName(comm->proxyState->thread, "NCCL Service %2d",
                      comm->cudaDev);

    // UDS support
    INFO(NCCL_PROXY, "UDS: Creating service thread comm %p rank %d", comm,
         comm->rank);
    comm->proxyState->threadUDS =
        std::thread(ncclProxyServiceUDS, comm->proxyState);
    ncclSetThreadName(comm->proxyState->threadUDS, "NCCL UDS Service %2d",
                      comm->cudaDev);
  }
  return ncclSuccess;
}
...
ncclResult_t ncclProxyStop(struct ncclComm *comm) {
  if (comm->proxyState) {
    ...
    if ((comm->proxyRefCountOld =
             ncclAtomicRefCountDecrement(&sharedProxyState->refCount)) == 0) {
        ...
        for (int i = 0; i < peerArraySize; i++) {
          ...
          if (fd != NCCL_INVALID_SOCKET) {
            ...
            if (sharedProxyState->sharedDevMems[i]) {
              if (!ncclCuMemEnable()) {
                CUDACHECK(
                    cudaIpcCloseMemHandle(sharedProxyState->sharedDevMems[i]));
              }
            }
            int type = ncclProxyMsgClose;
            (void)ncclSocketSend(sharedProxyState->peerSocks + i, &type,
                                 sizeof(int));
      ...
      COMPILER_ATOMIC_STORE(&comm->proxyState->stop, 1,
                            std::memory_order_release);
    }
  }
